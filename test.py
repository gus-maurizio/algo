...
with open(WORDS) as f:
    originalwords = [line.rstrip().lower() for line in f]

selection = {}
for word in originalwords[0:]:
    selection[word] = [w for w in originalwords  if not any([l in word for l in w])]
    if len(selection[word]) < 200:
        print(f'{word} {len(selection[word])}')




# # compute first,last AND letter probability
# origdistro = {k: [0,0,0] for k in allchr}
# for word in originalwords:
#     origdistro[word[0]][0] += 1
#     origdistro[word[-1]][1] += 1
#     for letter in word:
#         origdistro[letter][2] += 1

# wordScore = {}
# for word in originalwords:
#     wordScore[word] = [sum([origdistro[letter][2] for letter in set(word)]), origdistro[word[0]][0], origdistro[word[-1]][1]]
# sortedWordScore = {k: v for k, v in sorted(wordScore.items(), key=lambda item: item[1][0]+1*item[1][1]+item[1][2], reverse=True)}
# MAXBEST = 20
# count   = 0
# for k,v in sortedWordScore.items():
#     print(f'{k}: {v}')
#     count += 1
#     if count >= MAXBEST:
#         break
...
#         paralleresults = p.map(fullGame, originalwords[0:3])
...
#     maxresults = min(len(originalwords),4)
#     count = 0
#     MAXBEST = 20
