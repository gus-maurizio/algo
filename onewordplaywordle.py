...
def bestNext(words, guessfilter):
    ...
    for word in words:
        ...
        # print(f'{word} {wordScore[word]} {set(missing)}')
...
wordleword  = None      if len(sys.argv) <=  1 else sys.argv[1].lower()
startGuess  = 'alter'   if len(sys.argv) <=  2 else sys.argv[2].lower()
# startGuess   = 'scion'
if wordleword not in originalwords:
    print(f'{wordleword} not in word dictionary.')
    sys.exit(1)
if startGuess not in originalwords:
    print(f'{startGuess} not in word dictionary.')
    sys.exit(1)


# we need to discover the word, starting with the startGuess
guesses = [startGuess]
iterats = 1
guess   = startGuess
MAXITERATS = 20
words   = originalwords
print(f'----- [{wordleword}] Guess: _____->{guess} (out of {len(words)} words)')
...
while  guess != wordleword and guess != '' and iterats < MAXITERATS:
    ...
    print(f'----- [{wordleword}] Guess: {prevguess}->{guess} (out of {len(words):4d} words) resulted in: {guessfilter:14s} Letters not in positions: {letterNotInPosition}')
    iterats += 1
    if guess != wordleword:
        words = set(words) - {guess}
print(f'Game ended in {iterats} iterations: {guesses}')
