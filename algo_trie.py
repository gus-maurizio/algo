...
all_words_freq = [random.randint(0,100) for _ in all_words]
...
for idx,w in enumerate(all_words):
    ...
    for c in w:
        prefix += c
        if prefix not in trie_prefix:
            trie_prefix[prefix] = 1
        else:
            trie_prefix[prefix] += 1
print(len(trie_prefix.keys()))
print(len([k for k,v in trie_prefix.items() if v > 1]))
sortedResults = {k: v for k, v in sorted(trie_prefix.items(), key=lambda item: item[1],reverse=True) if len(k)> 1}
count = 0
for k in sortedResults.keys():
    print(k,sortedResults[k])
    count += 1
    if count > 20:
        break
